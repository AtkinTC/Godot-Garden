...
static func get_modified_purchase_price(category : String, key : String, _props : Dictionary = {}):
	...
	var props = {
		Const.MODIFIER_TARGET_CATEGORY : _props.get(Const.MODIFIER_TARGET_CATEGORY, category),
		Const.MODIFIER_TARGET_KEY : _props.get(Const.MODIFIER_TARGET_KEY, key),
		Const.MODIFIER_TYPE : _props.get(Const.MODIFIER_TYPE, Const.PRICE),
	...
	for supply_key in base_price.keys():
		...
		var supply_prop := {
			...
			Const.MODIFIER_TYPE : Const.PRICE
		}
...
static func get_local_price_modifier(category : String, key : String, props : Dictionary):
	...
	for modifier in modifiers:
		if(!modifier.has(Const.PRICE_MODIFIER_TARGET) || !modifier.has(Const.PRICE_MODIFIER_TYPE)):
		...
		var type : String = modifier.get(Const.PRICE_MODIFIER_TYPE)
