...
@export var custom_layer_names : Array[String] = []

var layers_by_name : Dictionary

var tile_defs : Array[TileDefinition]
...
#					custom_data[custom_data_layer_name] = custom_data_layer_data
				...
				var tile_def : TileDefinition = TileDefinition.new(source_id, tile_id, alt_id, custom_data)
...
func get_tile_identifier_for_cell(coord: Vector2i, layer_name : String = "") -> TileDefinition:
	...
	for tile_def in tile_defs:
		if(tile_def.source_id == source_id && tile_def.tile_id == tile_id && tile_def.alt_id == alt_id):
			return tile_def
	return null
